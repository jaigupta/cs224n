...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "LA":
            ...
            assert transition == "S" and len(self.buffer) > 0
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        minibatch_size = min(batch_size, len(unfinished_parses))
        minibatch_parses = unfinished_parses[-minibatch_size:]
        transitions = model.predict(minibatch_parses)
        for i in range(minibatch_size):
            parser = minibatch_parses[i]
            parser.parse_step(transitions[i])
            if len(parser.buffer) == 0 and len(parser.stack) == 1:
                unfinished_parses.pop(i - minibatch_size)
